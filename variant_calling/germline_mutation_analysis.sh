...
for list_R1 in $(find $dir -name "*R1*_paired.fastq.gz"|sort); do
    ...
    if [ ! -e "${sample_name}.sam" ] && [ ! -e "${sample_name}.bam" ]; then
        bwa mem -M -t 20 -R $header $ref_file ${list_R1}.fastq.gz ${list_R2}.fastq.gz > ${sam_output}.sam
    fi
done
...
for pass_vcfs in $(find $dir -name "*_pass.vcf");do
    ...
    if [ ! -e "${out}_final_hg38_selected.intervar" ]; then
        awk '{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$8"\t"$89"\t"$154}' ${out}_final.hg38_multianno.txt |\
        ...
        join -t $'\t' -1 1 -2 1 ${out}_annovar_wanted_header.tmp ${out}_intervar_wanted_header.tmp > ${out}_final_header.tmp
        join -t $'\t' -1 1 -2 1 ${out}_annovar_wanted_noheader.tmp ${out}_intervar_wanted_noheader.tmp > ${out}_final_noheader.tmp
